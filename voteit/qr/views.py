...
class QRViews(BaseView, QRViewMixin):
    ...
    @view_config(name="meeting_qr_userid.svg", permission=security.VIEW, http_cache=0)
    def userid_svg(self):
        payload = {"userid": self.request.authenticated_userid}
    ...
    def receiver(self):
            ...
            body = self.presence_qr.encode(
                {
                    ...
                    "config": {
                        "default_text": translate(
                            _("Please scan QR code to check in or out")
                        ),
        ...
        if userid not in self.presence_qr:
            response["message"] = translate(
                _("You've checked in as ${userid}", mapping={"userid": userid})
            )
            ...
            if value:
                if value == "yes":
                    ...
                    response["message"] = translate(
                        _("${act} is not a valid action", mapping={"act": value})
                    )
            else:
                response["question"] = {
                    "text": translate(
    ...
    def user_check_page(self):
        return {
            "qr_img_url": self.request.resource_url(
                self.context, "meeting_qr_userid.svg"
            ),
            "checked_in": self.request.authenticated_userid in self.presence_qr,
            "status_url": self.request.resource_url(
                self.context, "my_checkin_status.json"
            ),
        }

    @view_config(name="user_checkout", permission=security.VIEW)
    ...
    def register_endpoint_page(self):
        return {
            "qr_img_url": self.request.resource_url(
                self.context, "meeting_qr_endpoint.svg"
            )
        }
    ...
    def checked_in_users(self):
        ...
        for userid in self.presence_qr:
            try:
                ...
                user = None
            row = {"userid": userid, "fullname": user and user.title or ""}
            if pns:
...
class QRManualCheckin(DefaultEditForm, QRViewMixin):
    ...
    def get_user_groups(self, userid):
        ...
        try:
            ...
            return [
                {
                    "title": g.title,
                    "role": self.request.localizer.translate(
                        self.role_dict.get(g[userid]) or g[userid]
                    ).lower(),
    ...
    def checkin_success(self, appstruct):
        ...
        if pn and not ticket:
            self.flash_messages.add(
                _("Participant number is not available"), type="warning"
            )
        ...
        if ticket:
            if self.participant_numbers.userid_to_number.get(userid):
                ...
                try:
                    ...
                    self.flash_messages.add(
                        _("Participant number already claimed"), type="warning"
                    )
    ...
    def status_success(self, appstruct):
        ...
        url = self.request.resource_url(
            self.context, "manual_checkin", query={"userid": userid}
        )
...
class CheckoutEveryoneForm(DefaultEditForm, QRViewMixin):
    ...
    def save_success(self, appstruct):
        ...
        return HTTPFound(
            location=self.request.resource_url(self.context, "checked_in_users")
        )
...
def includeme(config):
    ...
    config.add_view_action(
        meeting_nav_link, "nav_meeting", "qr_check_user", permission=security.VIEW
    )
